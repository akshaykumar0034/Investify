...
import { 
  getMarketStatus, 
  getMarketIndices, 
  getQuote  // This is the Finnhub getQuote
...
import MarketIndicesList from '../components/market/MarketIndicesList'; 
...
function HomePage() {
  ...
  useEffect(() => {
    const fetchPublicData = async () => {
      try {
        const [statusRes, indicesRes] = await Promise.all([
          getMarketStatus(),
          getMarketIndices(), // Fetches NIFTY/SENSEX
        ]);
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        // --- 3. THIS IS THE FIX ---
        // Fetch portfolio quotes using the correct Finnhub logic
        if (portfolioRes.data.holdings && portfolioRes.data.holdings.length > 0) {
          ...
          quoteResponses.forEach((response, index) => {
            ...
            // Finnhub returns an object, not an array
            if (response.data) {
              marketDataMap[ticker] = { c: response.data.c }; // Use .c
            }
          });
  ...
  return (
    <div className="p-8 text-white">
      ...
      {/* --- Hero or Portfolio/Watchlist (Unchanged) --- */}
      <div className="mb-8">
        {loading ? (
      ...
      {/* --- Market Indices (Unchanged) --- */}
      <div className="mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {loading ? (
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg text-gray-400">Loading Market Data...</div>
          ) : (
